...
export default function App() {
  ...
  const [page, setPage] = useState<"main" | "compare">("main");
  ...
  const beforeImagePath = selectedGroup?.images[0]?.path ?? null;
  const beforeUrl = beforeImagePath ? convertFileSrc(beforeImagePath) : null;
  ...
  return (
    <div className="app">
      <div className="header">
        ...
        <div className="row">
          <button
            className={`button secondary ${page === "main" ? "active" : ""}`}
            onClick={() => setPage("main")}
          >
            メイン
          </button>
          <button
            className={`button secondary ${page === "compare" ? "active" : ""}`}
      ...
      {page === "main" ? (
        <>
          <div className="card">
            ...
            <div className="muted">
              連続撮影の判定: {GROUP_WINDOW_MS / 60000}分以内
            </div>
          </div>
        ...
        <div className="card">
          ...
          {beforeUrl && afterUrl ? (
            <div className="compare-wrapper" style={{ marginTop: 12 }}>
              <div className="compare-frame">
                <img className="compare-base" src={beforeUrl} alt="Before" />
                <div
                  className="compare-overlay"
                  style={{ width: `${compareValue}%` }}
                >
                  <img className="compare-after" src={afterUrl} alt="After" />
                </div>
              </div>
              <input
